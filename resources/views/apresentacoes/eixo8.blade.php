...
@section('content')
  ...
  @if(auth()->user()->unidade->unidadeTipoFK == 1 || auth()->user()->unidade->unidadeTipoFK == 3 || auth()->user()->unidade->unidadeTipoFK == 4)
    {{-- Container Flex único para todos os botões ficarem perfeitamente na mesma linha --}}
    <div class="d-flex justify-content-center gap-2 flex-wrap mb-4">
      <a href="{{ route('relatorios.eixos', ['id' => 8]) }}" class="highlighted-btn-sm highlight-blue text-decoration-none">
      ...
      <a href="{{ route('graficos.index') }}" class="highlighted-btn-sm highlight-blue text-decoration-none d-inline-flex align-items-center gap-1">
      ...
      <a href="{{ route('anexo', $eixo_id ?? 8) }}" class="highlighted-btn-sm highlight-blue text-decoration-none">
    ...
    <div class="modal fade" id="modalUploadAnexo" tabindex="-1" aria-labelledby="modalUploadAnexoLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <form action="{{ route('anexo.upload', $eixo_id ?? 8) }}" method="POST" enctype="multipart/form-data">
            @csrf
