    ...
    <div class="row d-flex justify-content-center align-items-center mt-5">
        <div class="col-lg-12">
            <div class="card rounded-3 shadow-sm border-1 mb-4">
                <div class="card-body">
                    <h2 class="mb-0 fw-bold">Editar Atividade</h2>
                </div>
            </div>
                    ...
                    <form action="{{ route('atividades.update', $atividade->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="eixo_id" id="eixo_id" class="form-select" required>
                                ...
                                @foreach ($eixos as $eixo)
                                    <option value="{{ $eixo->id }}" {{ $atividade->eixo_id == $eixo->id ? 'selected' : '' }}>
                                        {{ $eixo->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="tipo_evento" id="tipo_evento" class="form-select" required>
                                <option value="Presencial" {{ $atividade->tipo_evento == 'Presencial' ? 'selected' : '' }}>Presencial</option>
                                <option value="TeleConferência" {{ $atividade->tipo_evento == 'TeleConferência' ? 'selected' : '' }}>TeleConferência</option>
                            </select>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="d-flex justify-content-end">
                            <div class="mb-4">
                                <button type="submit" class="btn btn-md btn-primary">Atualizar Atividade</button>
                            </div>
                        </div>
                    </form>
                </div>
