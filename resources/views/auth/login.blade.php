@extends('layouts.app')
@section('title') {{ 'Bem vindo(a)!' }} @endsection
    ...
    <div class="order__left centered">
      <div class="form">
        <form method="POST" action="{{ route('login') }}">
          ...
          <!-- CAMPO CPF (Apenas valida erro de cpf) -->
          <div class="input-icon">
            <input id="cpf" type="tel" class="inputImp @error('cpf') is-invalid @enderror" name="cpf"
              value="{{ old('cpf') }}" placeholder="Insira seu CPF" autocomplete="cpf" required>
            <i class="fas fa-user"></i>

            @error('cpf')
              <span class="invalid-feedback" role="alert" style="display: block; color: red; font-size: 12px; margin-top: 5px;">
                <span class="fw-bold">{{ $message }}</span>
              </span>
            @enderror
          </div>

          <!-- CAMPO SENHA (Apenas valida erro de password) -->
          <div class="input-icon" style="margin-top: 1rem;">
            <input id="password" type="password" class="inputImp inputSenha @error('password') is-invalid @enderror"
        ...
        <div class="footerLogo">
          <span>&copy;{{ date('Y') }} FAPEAM - Versão 1.0</span>
        </div>

        <!-- 
        <div class="version">
          <span>
            <a href="{{ route('versionamentos.public') }}" style="color: #152d6e; text-decoration: none;">Ver últimas
              atualizações do sistema</a>
          </span>
