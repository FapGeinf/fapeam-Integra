...
<script src="{{ asset('js/prazo/insertPrazo.js') }}"></script>
<script src="{{ asset('js/notifications/handleNotifications.js') }}"></script>
<style>
    .liDP {
            ...
            $(document).ready(function () {
                ...
                var table = $('#tableHome').DataTable({
                    ...
                    initComplete: function () {
                        ...
                        var divContainer = $(
                            '<div class="divContainer d-flex justify-content-between align-items-center flex-wrap"></div>'
                        );
                        ...
                        var actionDropdownButton = $(
                            '<button class="btnAdd dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Ações</button>'
                        );
                        var actionDropdownMenu = $(
                            '<ul style="min-height: 97px;" class="dropdown-menu text-center p-3"></ul>');

                        @if (Auth::user()->unidade->unidadeTipoFK !== 2 || Auth::user()->unidade->unidadeTipoFK !== 5)
                            var newRiskButton = $(
                                '<li style="margin-left: 0; "><a href="{{ route('riscos.create') }}" class="btnAdd text-decoration-none"><i class="bi bi-plus-lg"></i> Novo Risco</a></li>'
                            );
                            var insertDeadlineButton = $(
                                '<li style="margin-left: 0;"><button type="button" class="mt-2 green-btn" data-bs-toggle="modal" data-bs-target="#prazoModal"><i class="bi bi-plus-lg"></i> Inserir Prazo</button></li>'
                            );
                        ...
                        var dropdownButton = $(
                            '<button class="btnFilter dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Abrir filtros</button>'
                        );
                        ...
                        if (!$('#filterUnidade').length) {
                            ...
                            @endforeach
                                ...
                                var labelUnidades = $(
                                '<label for="filterUnidade" class="labelUnidade d-block">Unidades:</label>'
                            );
                        ...
                        if (!$('#filterAvaliação').length) {
                            ...
                            var selectAvaliacao = $(
                                '<select id="filterAvaliação" class="form-select form-select-sm FilterAvaliacao"><option value="">Todas as Avaliações</option></select>'
                            );
                            ...
                            var labelAvaliacoes = $(
                                '<label for="filterAvaliação" class="labelAvaliação d-block">Avaliação:</label>'
                            );
                        ...
                        var prazoContainer = $(
                            '<p class="spanThatLooksLikeABtn" id="prazo" data-prazo="{{ \Carbon\Carbon::parse($prazo)->format('Y-m-d') }}">Prazo Final: <strong>{{ \Carbon\Carbon::parse($prazo)->format('d/m/Y') }}</strong></p>'
                        );
