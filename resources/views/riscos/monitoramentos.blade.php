...
<div class="form-wrapper pt-4">
    <div class="form_create">
        ...
        <form action="{{ route('riscos.insert-monitoramentos', ['id' => $risco->id]) }}" method="POST" id="formCreate" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            @foreach ($risco->monitoramentos as $index => $monitoramento)
                <div class="monitoramento">
                    ...
                    <textarea name="monitoramentos[{{ $index }}][monitoramentoControleSugerido]" placeholder="Monitoramento"
                        class="textInput" id="monitoramentoControleSugerido{{ $index }}">{{ $monitoramento->monitoramentoControleSugerido }}</textarea>

                    <div class="row g-3">
                        <div class="col-sm-6 col-md-6">
                            <label>Status do Monitoramento:</label>
                            <select name="monitoramentos[{{ $index }}][statusMonitoramento]"
                                class="textInput form-select" id="statusMonitoramento{{ $index }}">
                                <option value="NÃO IMPLEMENTADA"
                                    {{ $monitoramento->statusMonitoramento == 'NÃO IMPLEMENTADA' ? 'selected' : '' }}>
                                    NÃO IMPLEMENTADA</option>
                                <option value="EM IMPLEMENTAÇÃO"
                                    {{ $monitoramento->statusMonitoramento == 'EM IMPLEMENTAÇÃO' ? 'selected' : '' }}>
                                    EM IMPLEMENTAÇÃO</option>
                                <option value="IMPLEMENTADA PARCIALMENTE"
                                    {{ $monitoramento->statusMonitoramento == 'IMPLEMENTADA PARCIALMENTE' ? 'selected' : '' }}>
                                    IMPLEMENTADA PARCIALMENTE</option>
                                <option value="IMPLEMENTADA"
                                    {{ $monitoramento->statusMonitoramento == 'IMPLEMENTADA' ? 'selected' : '' }}>
                                    IMPLEMENTADA</option>
                            </select>
                        </div>
                        ...
                        <div class="col-sm-6 col-md-6">
                            ...
                            <select name="monitoramentos[{{ $index }}][isContinuo]" class="form-select"
                                id="isContinuo{{ $index }}">
                                <option value="1" {{ $monitoramento->isContinuo == 1 ? 'selected' : '' }}>Sim
                                </option>
                                <option value="0" {{ $monitoramento->isContinuo == 0 ? 'selected' : '' }}>Não
                                </option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-sm-12 col-md-6">
                            ...
                            <input type="date" name="monitoramentos[{{ $index }}][inicioMonitoramento]"
                                class="textInput bgDateMon dateInput"
                                value="{{ $monitoramento->inicioMonitoramento }}">
                        </div>

                        <div class="col-sm-12 col-md-6 mQuery2" id="fimMonitoramentoContainer{{ $index }}">
                            <label>Fim do Monitoramento:</label>
                            <input type="date" name="monitoramentos[{{ $index }}][fimMonitoramento]"
                                class="textInput dateInput" id="fimMonitoramento{{ $index }}"
                                value="{{ $monitoramento->fimMonitoramento }}">
                        </div>
                    </div>

                    <div class="form-group mb-4">
                        <label>Anexo:</label>
                        <input type="file" name="monitoramentos[{{ $index }}][anexoMonitoramento]" class="form-control" value="">
                        <p>{{$monitoramento->anexoMonitoramento}}</p>
                    </div>
                ...
                <script>
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        let fimMonitoramentoContainer = document.getElementById(
                        'fimMonitoramentoContainer{{ $index }}');
...
<script>
    ...
    // Toggle visibility of "Fim do Monitoramento" based on "Monitoramento Contínuo" selection
    ...
    @foreach ($risco->monitoramentos as $index => $monitoramento)
        ...
        let fimMonitoramento{{ $index }} = document.getElementById(
            `fimMonitoramentoContainer{{ $index }}`);
    ...
    function showConfirmationModal() {
        ...
        monitoramentos.forEach((monitoramento, index) => {
            let monitoramentoControleSugerido = CKEDITOR.instances[`monitoramentoControleSugerido${index}`]
                .getData();
            let statusMonitoramento = monitoramento.querySelector(
                `select[name^="monitoramentos[${index}][statusMonitoramento]"]`).value;
            let isContinuo = monitoramento.querySelector(`select[name^="monitoramentos[${index}][isContinuo]"]`)
                .value;
            let inicioMonitoramento = monitoramento.querySelector(
                `input[name^="monitoramentos[${index}][inicioMonitoramento]"]`).value;
            let fimMonitoramento = monitoramento.querySelector(
                `input[name^="monitoramentos[${index}][fimMonitoramento]"]`).value;
