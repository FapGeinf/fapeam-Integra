        ...
        @foreach ($respostas as $resposta)
          ...
          @if ($lastSetor === $currentSetor)
            <div class="message {{ $isLeftAligned ? 'other-message' : 'another-message' }} {{ $alignmentClass }}">
                  ...
                  <div class="dataSector" style="text-align: left; {{ $alignmentClass === 'align-right' ? 'background-color: #d9d9d9cc;' : '' }}">
                    <div>
                      ...
                      <span class="dataSpan text-uppercase">
                        {{ $resposta->created_at->format('d/m/Y') }} às {{ $resposta->created_at->format('H:i') }}
                      </span>
                    </div>
              ...
              @if ($resposta->anexo)
                <div class="dropdown">
                  <button
                    class="highlighted-btn highlight-grey dropdown-toggle"
                    type="button"
                  ...
                  <ul class="dropdown-menu dropdown-menu-dark">
                    <li>
                      <a class="dropdown-item a-navbar" href="{{ Storage::url($resposta->anexo) }}" target="_blank">
                        <i class="bi bi-eye me-2"></i>Visualizar
                      </a>
                    </li>

                    <li>
                      <a class="dropdown-item a-navbar" href="{{ Storage::url($resposta->anexo) }}" download>
          ...
          @else
            ...
            <div class="message {{ $alignmentClass === 'align-left' ? 'other-message' : 'another-message' }} {{ $alignmentClass }}">
                ...
                <div class="d-flex row">
                  <div class="dataSector" style="text-align: left; {{ $alignmentClass === 'align-right' ? 'background-color: #d9d9d9cc;' : '' }}">
                    <div>
                      ...
                      <i class="bi bi-clock"></i>
                      <span class="dataSpan text-uppercase">
                        {{ $resposta->created_at->format('d/m/Y') }} {{ $resposta->created_at->format('H:i') }}
                      </span>
                    </div>
              ...
              @else
                <div class="text-end">
                  <button type="button" class="highlighted-btn highlight-warning me-1" data-bs-toggle="modal" data-bs-target="#editRespostaModal" 
                    onclick="editResposta({{ $resposta->id }}, `{{ $resposta->respostaRisco }}`)">
                    <i class="bi bi-pen me-1"></i>Editar
                  </button>
      ...
      <div class="modal-body">
        <form action="{{ route('riscos.storeResposta', ['id' => $monitoramento->id]) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="mb-4">
            ...
            <select class="form-select input-enabled pointer" id="statusMonitoramento" name="statusMonitoramento" required>
