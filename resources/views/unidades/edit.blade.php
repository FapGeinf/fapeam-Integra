@extends('layouts.app')
@section('title') {{ 'Editar Unidade' }} @endsection
@section('content')
...
<div class="container pt-5" style="max-width: 680px;">
  <div class="col-12 border box-shadow">
    <h5 class="text-center">Editar Unidade</h5>
    ...
    <form action="{{ route('unidades.update', $unidade->id) }}" method="POST" id="formStoreUnidade">
      @csrf
      @method('PUT')
      ...
      <div class="row g-3">
        <div class="col-12">
          ...
          <input type="text" class="form-control input-enabled" id="unidadeNome" name="unidadeNome" 
                 value="{{ $unidade->unidadeNome }}" required>
        </div>
        ...
        <div class="col-12">
          ...
          <input type="text" class="form-control input-enabled" id="unidadeSigla" name="unidadeSigla" 
                 value="{{ $unidade->unidadeSigla }}" required>
        </div>
        ...
        <div class="col-12">
          ...
          <input type="email" class="form-control input-enabled" id="unidadeEmail" name="unidadeEmail" 
                 value="{{ $unidade->unidadeEmail }}" required>
        </div>
        ...
        <div class="col-12">
          ...
          <select class="form-select input-enabled border-grey pointer" id="unidadeTipoFK" name="unidadeTipoFK" required>
            @foreach($unidadeTipos as $tipo)
              <option value="{{ $tipo->id }}" {{ $unidade->unidadeTipoFK == $tipo->id ? 'selected' : '' }}>
                {{ $tipo->unidadeTipoNome }}
              </option>
            @endforeach
          </select>
        </div>
